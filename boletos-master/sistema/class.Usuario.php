  ...
  class Usuario {
    ...
    public function setAlias ($alias) {
      if (!ctype_alnum($alias)) {
        throw new Exception('El usuario debe ser alfanumerico.', 1);
      }
      
      $longitud = strlen($alias);
      
      if ($longitud < self :: MIN_LONG_ALIAS || $longitud > self :: MAX_LONG_ALIAS) {
        throw new Exception('usuario muy corto o largo.', 2);
      }
    ...
    public function setPassword ($password, $confirmarPassword) {
      if ($password != $confirmarPassword) {
        throw new Exception('Las contrasenas no coinciden.', 1);
      }
      
      $longitud = strlen($password);
      
      if ($longitud < self :: MIN_LONG_PASSWORD) {
        throw new Exception('La contrasena es muy corta.', 2);
      }
    ...
    public function setCedula ($cedula) {
      ...
      if (!is_numeric($cedula)) {
        throw new Exception('El DNI debe ser un numero.', 1);
      }
      
      if ($cedula < self :: CEDULA_MIN || $cedula > self :: CEDULA_MAX) {
        throw new Exception('DNI invalido.', 2);
      }
